...
class EmployeePage(BasePage):
    ...
    def view_payslip(self, emp_name, sheet_name, rownum):
        try:
            self.select_from_table(self.all_emp, emp_name)
            self.wait_till_click(self.edit)
            self.click(self.payslip_btn)
            self.wait_till_click(self.create)
            self.click(self.emp_name_in_table)
            self.click(self.payslip_name)
            self.wait_till_click(self.edit)
            self.click(self.print_btn)
            self.click(self.payslip)
            time.sleep(60)
            name = self.getText(self.name)
            assert name + " - Odoo" == self.driver.title
            ScreenShot.takeScreenshot(self.driver, name)
            if rownum != 0:
                ExcelUtil.write_data(PropertyFile.getValues('test_report_file'),
                                     sheet_name, rownum, 6,
            ...
            ScreenShot.takeScreenshot(self.driver, 'Error while downloading payslip ')
            if rownum != 0:
                ExcelUtil.write_data(PropertyFile.getValues('test_report_file'),
                                     sheet_name, rownum, 6,
                                     "Error while downloading Payslip")
                ExcelUtil.write_data(PropertyFile.getValues('test_report_file'),
                                     sheet_name, rownum, 7,
                                     'Pass')
            return None
