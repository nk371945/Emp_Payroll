...
class TestData:
    ...
    def getTestData(sheet):
        dataList = []
        filePath = PropertyFile.getValues('excelFilePath')

        if sheet == 'Test Data For Login':
            testSheet = PropertyFile.getValues('login_test_data')
            fname = "login"
            ...
            testSheet = PropertyFile.getValues('department')
            fname = "department"
            rowCount = ExcelUtil.get_rowcount(filePath, testSheet)
            ...
            testSheet = PropertyFile.getValues('employee')
            fname = "employee"
            rowCount = ExcelUtil.get_rowcount(filePath, testSheet)
            ...
            testSheet = PropertyFile.getValues('payroll')
            fname = "payroll"
            rowCount = ExcelUtil.get_rowcount(filePath, testSheet)
            ...
            testSheet = PropertyFile.getValues('view_payslip')
            fname = "payslip"
            rowCount = ExcelUtil.get_rowcount(filePath, testSheet)
        ...
        destination_file = PropertyFile.getValues('test_report_file')
